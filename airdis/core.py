...
def read_places_from_csv(
    path: str,
) -> Union[Tuple[List[Union[str, float]]], ExitStatus]:
    """
...
def print_average_distance_and_closest_pairs(
    places: List[Union[str, float]]
) -> ExitStatus:
    try:
        average_distance = calculate_average_distance(places)
        closest_pair = find_the_closest_pair(places, average_distance)
        print(
            "Average distance: {:.2f} km. Closest pair: {} - {} {:.2f} km".format(
                average_distance, closest_pair[0], closest_pair[1], closest_pair[2]
            )
        )
        return ExitStatus.SUCCESS
    except:
        print(
            "ERROR: Something wrong happened while printing the average distance and closest pairs!"
        )
        return ExitStatus.ERROR
...
def main() -> ExitStatus:
    ...
    data = sort_places_by_distance(
        get_random_places_list(csv_data, n), calculate_haversine_distance
    )
